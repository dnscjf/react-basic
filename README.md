...
- 리액트 프로젝트에서 전역적으로 사용할 데이터가 있을 때 유용한 기능 **유용한 기능**
- 상태관리 도구는 아닙니다.
...
function App() {
  return (
    <colorContext.Provider value={{ color: "gold" }}>
      <div>
        <ColorBox />
      </div>
...
export const { Consumer: colorCosumer } = colorContext;
```
...
- Context의 actions에 넣어준 함수를 호출하는 컴포넌트
