...
- https://developer.mozilla.org/ko/docs/web/javaScript/Reference.Hlobal_objects/Array/map
...
const Main = () => {
  // member 목록 상태
  const [members, setMemberInfo] = useState(initState);
  // member.id 상태
...
- push가 아닌 concat을 사용하자. 이유는 불변성 유지(imutable)
- 리액트에서는 상태를 업데이트할 때 기존 상태를 그대로 두면서 새로운 값을 설정해야합니다.(불변성유지)
- push는 기존의 배열 자체를 변경
